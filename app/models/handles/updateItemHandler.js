...
class updateItemHandler extends BaseClass {
	constructor() {
		super();
	}
	...
	async handler(ctx, next) {
		...
		let id = parseFloat(ctx.request.query.id);
		let playerRes = await this.groupModel.updateNewGroup(name,score,id);
		runDate = await this.run(ctx, next,playerRes);
		// 新用户存一下cookie
